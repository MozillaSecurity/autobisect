...
def captureStdout(inputCmd, ignoreStderr=False, combineStderr=False, ignoreExitCode=False,
    ...
    if p.returncode != 0:
        ...
        if not ignoreExitCode:
            ...
            if 'no such option: -s' not in stdout:
                log.info('Nonzero exit code from: %s' % shellify(cmd))
                log.info('stdout is: %s' % stdout)
            if stderr is not None:
                log.info('stderr is: %s' % stderr)
            # Pymake in builds earlier than revision 232553f741a0 did not support the '-s' option.
    ...
    if not combineStderr and not ignoreStderr and len(stderr) > 0:
        ...
        if not (isWin and 'warning: failed to set color mode to win32' in stderr):
            log.error('Unexpected output on stderr from: %s' % shellify(cmd))
            log.error(stdout)
            log.error(stderr)
    ...
    if stderr and ignoreStderr and len(stderr) > 0 and p.returncode != 0:
        ...
        log.error('Return code not zero, and unexpected output on stderr from: %s' % shellify(cmd))
        log.error(stdout)
        log.error(stderr)
...
def grabCrashLog(progfullname, crashedPID, logPrefix, wantStack):
    ...
    if debuggerCmd:
        ...
        if debuggerExitCode != 0:
            log.info('Debugger exited with code %d : %s' % (debuggerExitCode, shellify(debuggerCmd)))
        if useLogFiles:
            ...
            log.info("I don't know what to do with a core file when logPrefix is null")
    ...
    # On Mac, look for a crash log generated by Mac OS X Crash Reporter
        ...
        while True:
            ...
            if loops > maxLoops:
                ...
                log.info("grabCrashLog waited a long time, but a crash log for " + progname + " [" + str(
                    crashedPID) + "] never appeared!")
                break

    elif isLinux:
        log.info("Warning: grabCrashLog() did not find a core file for PID %d." % crashedPID)
        log.info("Note: Your soft limit for core file sizes is currently %d. You can increase it with 'ulimit -c' in bash." % getCoreLimit()[0])
...
def constructCdbCommand(progfullname, crashedPID):
    ...
    if not os.path.exists(cdbPath):
        log.info('WARNING: cdb.exe is not found - all crashes will be interesting.')
        return None
    ...
    if isWinDumpingToDefaultLocation():
        ...
        while True:
            ...
            if loops > maxLoops:
                ...
                log.info("constructCdbCommand waited a long time, but " + dumpFilename + " never appeared!")
                return None
    else:
        return None
...
def isWinDumpingToDefaultLocation():
    ...
    try:
        with _winreg.OpenKey(_winreg.ConnectRegistry(None, _winreg.HKEY_LOCAL_MACHINE),
            ...
            try:
                ...
                if not (dumpTypeRegValue[0] == 1 and dumpTypeRegValue[1] == _winreg.REG_DWORD):
                    log.info(noMinidumpMsg)
                    return False
            except WindowsError as e:
                if e.errno == 2:
                    log.error(noMinidumpMsg)
                    return False
                else:
                    raise
            ...
            try:
                ...
                if not (dumpFolderRegValue[0] == r'%LOCALAPPDATA%\CrashDumps' and
                    ...
                    log.info('WARNING: Dumps are instead appearing at: %s' % dumpFolderRegValue[0])
                    log.error('All crashes will be uninteresting.')
                    return False
            except WindowsError as e:
        ...
        if e.errno == 2 and e.strerror == 'The system cannot find the file specified':
            log.info('WARNING: The registry key HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\' + \
...
def handleRemoveReadOnly(func, path, exc):
    ...
    if func in (os.rmdir, os.remove) and exc[1].errno == errno.EACCES:
        ...
        raise Exception("Failed to remove read-only files")
...
def timeSubprocess(command, ignoreStderr=False, combineStderr=False, ignoreExitCode=False,
    ...
    """
    log.info('Running `%s` now..' % shellify(command))
    startTime = time.time()
    ...
    endTime = time.time()
    log.info('`%s` took %.3f seconds.\n' % (shellify(command), (endTime - startTime)))
